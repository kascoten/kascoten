// ============================
// Annual Events Page Functionality - MIRRORS NEWS PAGE
// ============================
...
function initializeEventsPage() {
  ...
  window.logContentReady('Events Grid');
}
...
function loadSampleArticles() {
  ...
  // Filter only annual events articles
  const eventArticles = sampleArticles.filter(article => article.category === 'annual-events');
  
  allArticles = eventArticles;
  if (eventArticles.length > 0) {
    const sorted = applySorting(eventArticles);
...
function renderArticles(articles) {
  ...
  eventsGrid.innerHTML = articles.map(article => createNewsCard(article)).join('');
...
function createNewsCard(article) {
  ...
  const categoryLabel = 'Annual Event';
      ...
      <div class="news-card-content">
        ...
        <div class="news-card-footer">
          ...
          <div class="news-card-actions">
            <button class="card-action-btn like-btn" data-article-id="${article.id}" title="Like this event">
...
function setupEventListeners() {
  // Sort dropdown
  const sortDropdown = document.getElementById('eventsSortDropdown');
  if (sortDropdown) {
